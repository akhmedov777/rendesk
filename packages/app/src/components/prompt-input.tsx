import { useFilteredList } from "@rendesk/ui/hooks"
import { useSpring } from "@rendesk/ui/motion-spring"
...
import { DockShellForm, DockTray } from "@rendesk/ui/dock-surface"
...
import { type ActiveEditorContext, usePlatform } from "@/context/platform"
...
import { ImagePreview } from "@rendesk/ui/image-preview"
...
const createEditorContextItem = (editor: ActiveEditorContext) => ({
...
const canIncludeEditorContext = (editor: ActiveEditorContext | null | undefined) => {
  if (!editor?.ready) return false
  if (editor.documentType === "cell") return !!editor.selectionRange?.range
  return editor.selectedText.trim().length > 0
}

const isMatchingEditorContext = (
  item: ReturnType<typeof createEditorContextItem> & { key?: string },
  editor: ActiveEditorContext | null | undefined,
) => {
  if (!editor || item.filePath !== editor.filePath || item.documentType !== editor.documentType) return false
  if (item.selectionRange || editor.selectionRange) {
    return (
      item.selectionRange?.range === editor.selectionRange?.range &&
      item.selectionRange?.sheetName === editor.selectionRange?.sheetName &&
      item.selectionRange?.preview === editor.selectionRange?.preview
    )
  }
  return item.selectedText.trim() === editor.selectedText.trim()
}
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const activeEditorContext = createMemo(() => platform.activeEditorContext?.() ?? null)
  const selectableEditorContext = createMemo(() => {
    const editor = activeEditorContext()
    return canIncludeEditorContext(editor) ? editor : null
  })
  const currentEditorContextKey = createMemo(() => {
    const editor = selectableEditorContext()
    if (!editor) return null
    const existing = prompt.context
      .items()
      .find((item) => item.type === "editor" && isMatchingEditorContext(item, editor))
    return existing?.key ?? null
  })
  const hasCurrentEditorContext = createMemo(() => currentEditorContextKey() !== null)
  ...
  const toggleEditorContext = () => {
    const editor = selectableEditorContext()
    const wasCurrent = hasCurrentEditorContext()
    const existingEditorItems = prompt.context.items().filter((item) => item.type === "editor")
    for (const item of existingEditorItems) {
      prompt.context.remove(item.key)
    }
    if (!editor || wasCurrent) return
    prompt.context.add(createEditorContextItem(editor))
  }
  ...
  const submitLocked = createMemo(() => sync.status === "loading")
      ...
      >
        ...
        <PromptContextItems
          ...
          active={(item) => {
            ...
            return (
              item.type === "file" && !!item.commentID && item.commentID === active?.id && item.path === active?.file
            )
          }}
          ...
          <div class="pointer-events-none absolute bottom-1.5 right-1.5 flex items-center gap-1.5">
            ...
            >
              ...
              <Show when={platform.platform === "desktop"}>
                <Tooltip
                  placement="top"
                  inactive={!selectableEditorContext()}
                  value={hasCurrentEditorContext() ? "Remove editor selection" : "Include editor selection"}
                >
                  <Button
                    type="button"
                    variant="ghost"
                    ...
                    disabled={store.mode !== "normal" || !selectableEditorContext()}
                    tabIndex={store.mode === "normal" ? undefined : -1}
                    aria-pressed={hasCurrentEditorContext()}
                    aria-label={hasCurrentEditorContext() ? "Remove editor selection" : "Include editor selection"}
                  >
                    <Icon
                      name="window-cursor"
                      class="size-4"
              ...
              >
                <IconButton
                  ...
                  type="submit"
                  disabled={
                    submitLocked() || store.mode !== "normal" || (!prompt.dirty() && !working() && commentCount() === 0)
                  }
                  tabIndex={store.mode === "normal" ? undefined : -1}
