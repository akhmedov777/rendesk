...
function shortArg(key: string, value: string | number | boolean) {
...
function args(input: Record<string, unknown> | undefined) {
  if (!input) return []
  const skip = new Set(["description", "query", "url", "filePath", "path", "pattern", "name", "content", "command", "oldString", "newString", "raw"])
  return Object.entries(input)
    .filter(([key]) => !skip.has(key))
    .flatMap(([key, value]) => {
      if (typeof value === "string") return shortArg(key, value) ? [shortArg(key, value)!] : []
      if (typeof value === "number") return shortArg(key, value) ? [shortArg(key, value)!] : []
      if (typeof value === "boolean") return shortArg(key, value) ? [shortArg(key, value)!] : []
      return []
    })
    .slice(0, 3)
}
...
function toolDisplayName(tool: string): string {
  ...
  return TOOL_DISPLAY_NAMES[normalized] || normalized.charAt(0).toUpperCase() + normalized.slice(1)
}
