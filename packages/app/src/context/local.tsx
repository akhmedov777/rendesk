...
import type { Agent } from "@rendesk/sdk/v2/client"
...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const agent = (() => {
      const fallback = createMemo<Agent | undefined>(() => {
        ...
        return {
          name: "Rendesk",
          description: "General Rendesk agent for business documents, spreadsheets, and PDF workflows.",
          mode: "primary",
          permission: [],
          model: {
            providerID,
      ...
      const list = createMemo(() => {
        const available = sync.data.agent.filter((x) => x.mode !== "subagent" && !x.hidden)
        if (available.length > 0) return available
        const synthetic = fallback()
    ...
    const model = (() => {
      ...
      const current = createMemo(() => {
        ...
        const key = getFirstValidModel(
          () => (a ? ephemeral.model[a.name] : undefined),
          () => a?.model,
          fallbackModel,
        )
