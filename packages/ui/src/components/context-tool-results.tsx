...
function contextToolLabel(part: ToolPart): { action: string; detail: string } {
  ...
  if (tool === "write") {
    ...
    return { action: "Write", detail: title || (path ? getFilename(path) : "") }
  }
  if (tool === "edit") {
    const path = input?.filePath as string | undefined
    return { action: "Edit", detail: title || (path ? getFilename(path) : "") }
  }
  if (tool === "bash") {
    const description = (input?.description as string | undefined) || (input?.command as string | undefined)
    return { action: "Terminal", detail: title || description || "" }
  }
  if (tool === "read") {
    const path = input?.filePath as string | undefined
  ...
  if (tool === "apply_patch") {
    ...
    return { action: "Patch", detail: title || (files > 0 ? `${files} file${files === 1 ? "" : "s"}` : "") }
  }
...
function actionToolSummary(parts: ToolPart[]) {
  const counts = new Map<string, number>()
  const order: string[] = []
  for (const part of parts) {
    const action = contextToolLabel(part).action
    if (!counts.has(action)) order.push(action)
    counts.set(action, (counts.get(action) ?? 0) + 1)
  }
  const headline = `${parts.length} action${parts.length === 1 ? "" : "s"}`
  const detail = order
    .slice(0, 3)
    .map((action) => `${counts.get(action) ?? 0} ${action.toLowerCase()}`)
    .join(" · ")
  return {
    headline,
    detail,
...
export function ActionToolGroupHeader(props: {
  parts: ToolPart[]
  pending: boolean
  open: boolean
  onOpenChange: (value: boolean) => void
}) {
  const summary = createMemo(() => actionToolSummary(props.parts))
  return (
    <ToolCall
      variant="row"
      icon="task"
      ...
      showArrow={!props.pending}
      onOpenChange={(value) => {
        if (!props.pending) props.onOpenChange(value)
      }}
      trigger={{
