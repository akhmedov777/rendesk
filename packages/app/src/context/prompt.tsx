...
import type { ActiveEditorContext } from "@/context/platform"
...
export type EditorContextItem = {
  ...
  fileName: string
  fileExt: string
  documentType: ActiveEditorContext["documentType"]
  selectedText: string
  selectionRange: ActiveEditorContext["selectionRange"]
}

export type ContextItem = FileContextItem | EditorContextItem
...
function contextItemKey(item: ContextItem) {
  if (item.type === "editor") {
    const selectionKey = item.selectionRange
      ? `${item.selectionRange.range}:${item.selectionRange.sheetName}`
      : (checksum(item.selectedText.trim())?.slice(0, 8) ?? "selection")
    return `editor:${item.filePath}:${selectionKey}`
  }
