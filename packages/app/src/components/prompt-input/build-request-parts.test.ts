...
describe("buildRequestParts", () => {
  ...
  test("serializes document editor selections as synthetic text context", () => {
    const result = buildRequestParts({
      prompt: [{ type: "text", content: "Summarize this", start: 0, end: 14 }],
      context: [
        {
          key: "editor:1",
          type: "editor",
          filePath: "/repo/docs/report.docx",
          fileName: "report.docx",
          fileExt: "docx",
          documentType: "word",
          selectedText: "Quarterly revenue increased by 12%.",
          selectionRange: null,
        },
      ],
      images: [],
      text: "Summarize this",
      messageID: "msg_editor_doc",
      sessionID: "ses_editor_doc",
      sessionDirectory: "/repo",
    })

    const editorPart = result.requestParts.find(
      (part) => part.type === "text" && part.synthetic && part.text.startsWith("[Editor Selection]"),
    )

    expect(editorPart).toBeDefined()
    if (editorPart?.type === "text") {
      expect(editorPart.text).toContain("Quarterly revenue increased by 12%.")
      expect(editorPart.metadata?.opencodeEditor).toMatchObject({
        filePath: "/repo/docs/report.docx",
        documentType: "word",
      })
    }
  ...
  test("serializes spreadsheet editor selections with range metadata", () => {
    const result = buildRequestParts({
      prompt: [{ type: "text", content: "Review the sheet", start: 0, end: 16 }],
      context: [
        {
          key: "editor:2",
          type: "editor",
          filePath: "/repo/docs/model.xlsx",
          fileName: "model.xlsx",
          fileExt: "xlsx",
          documentType: "cell",
      ...
      images: [],
      text: "Review the sheet",
      messageID: "msg_editor_sheet",
      sessionID: "ses_editor_sheet",
      sessionDirectory: "/repo",
    })

    const editorPart = result.requestParts.find(
      (part) => part.type === "text" && part.synthetic && part.text.startsWith("[Editor Context:"),
    )

    expect(editorPart).toBeDefined()
    if (editorPart?.type === "text") {
      expect(editorPart.text).toContain(`B2:C4 on "Forecast"`)
      expect(editorPart.text).toContain("120\t130")
      expect(editorPart.metadata?.opencodeEditor).toMatchObject({
        filePath: "/repo/docs/model.xlsx",
        range: "B2:C4",
        sheetName: "Forecast",
