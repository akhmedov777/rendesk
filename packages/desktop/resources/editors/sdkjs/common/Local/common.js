...
(function (window, undefined)
{
	...
	AscCommon.baseEditorsApi.prototype.onEndLoadFile = function(result)
	{
		...
		if (this.isLoadFullApi && this.DocInfo && this._isLoadedModules())
		{
			...
			window["_rendeskEditorInstance"] = this;
...
AscFonts.CFontFileLoader.prototype.LoadFontAsync = function(basePath, callback)
{
	...
	xhr.fontFile = this;
	var _serviceUrl = (window._RENDESK_SERVICE_URL || window.location.origin);
	xhr.open('GET', _serviceUrl + "/api/editor/fonts/file?path=" + encodeURIComponent(this.Id), true);
...
window["DesktopOfflineAppDocumentEndLoad"] = function(_url, _data, _len)
{
	...
	// NOTE(victor): Upstream checked indexOf("file:") which mangles http:// and
	// other valid URI schemes with a file:// prefix. RFC 3986 regex matches any
	// scheme (http:, https:, blob:, data:, etc.) so only bare paths get rewritten.
	if (!/^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(AscCommon.g_oDocumentUrls.documentUrl))
	{
