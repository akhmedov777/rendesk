...
export function applyDirectoryEvent(input: {
  ...
  switch (event.type) {
    ...
    case "message.part.updated": {
      ...
      input.setStore("part", part.messageID, (parts) => upsertMessagePart(parts, part))
      break
    }
    case "message.part.removed": {
      ...
      if (!parts) break
      const result = parts.findIndex((part) => part.id === props.partID)
      if (result === -1) break
      input.setStore(
        produce((draft) => {
          const list = draft.part[props.messageID]
          if (!list) return
          const next = list.findIndex((part) => part.id === props.partID)
          if (next === -1) return
          list.splice(next, 1)
