...
import type { Message, Part, PermissionRequest, Project, QuestionRequest, Session } from "@rendesk/sdk/v2/client"
...
const toolPart = (id: string, sessionID: string, messageID: string, start: number) =>
  ({
    id,
    sessionID,
    messageID,
...
describe("applyDirectoryEvent", () => {
  ...
  test("orders message parts by execution time and still supports updates by id", () => {
    const sessionID = "ses_1"
    const messageID = "msg_1"
    const [store, setStore] = createStore(
      baseState({
        part: {
          [messageID]: [
            {
              ...textPart("a_text", sessionID, messageID),
              text: "hello",
              time: { start: 20 },
            } as Part,
          ],
    ...
    applyDirectoryEvent({
      event: {
        type: "message.part.updated",
        properties: { part: toolPart("z_tool", sessionID, messageID, 10) },
      },
      store,
      setStore,
      push() {},
    ...
    expect(store.part[messageID]?.map((x) => x.id)).toEqual(["z_tool", "a_text"])

    applyDirectoryEvent({
      event: {
        type: "message.part.delta",
        properties: { messageID, partID: "a_text", field: "text", delta: " world" },
      },
      store,
      setStore,
      push() {},
    ...
    const text = store.part[messageID]?.find((part) => part.id === "a_text")
    expect(text?.type).toBe("text")
    if (text?.type === "text") expect(text.text).toBe("hello world")

    applyDirectoryEvent({
      event: { type: "message.part.removed", properties: { messageID, partID: "z_tool" } },
      store,
      setStore,
      push() {},
      directory: "/tmp",
    ...
    expect(store.part[messageID]?.map((x) => x.id)).toEqual(["a_text"])
  })
