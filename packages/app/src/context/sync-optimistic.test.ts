...
const toolPart = (id: string, sessionID: string, messageID: string, start: number): Part => ({
  id,
  sessionID,
  messageID,
  type: "tool",
...
describe("sync optimistic reducers", () => {
  ...
  test("applyOptimisticAdd stores parts in execution order", () => {
    const sessionID = "ses_1"
    const draft = {
    ...
    applyOptimisticAdd(draft, {
      sessionID,
      message: userMessage("msg_1", sessionID),
      parts: [textPart("a_text", sessionID, "msg_1"), toolPart("z_tool", sessionID, "msg_1", 10)],
    })

    expect(draft.part.msg_1?.map((x) => x.id)).toEqual(["z_tool", "a_text"])
  })
