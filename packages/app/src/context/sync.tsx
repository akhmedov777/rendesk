...
import type { Message, Part } from "@rendesk/sdk/v2/client"
...
export function applyOptimisticAdd(draft: OptimisticStore, input: OptimisticAddInput) {
  ...
  draft.part[input.message.id] = sortMessageParts(input.parts)
}
...
function setOptimisticAdd(setStore: (...args: unknown[]) => void, input: OptimisticAddInput) {
  ...
  setStore("part", input.message.id, sortMessageParts(input.parts))
}
...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const fetchMessages = async (input: { client: typeof sdk.client; sessionID: string; limit: number }) => {
      ...
      )) as { data?: Array<{ info: Message; parts: Part[] }> }
      const items = (messages.data ?? []).filter((x) => !!x?.info?.id)
      const session = items.map((x) => x.info).sort((a, b) => cmp(a.id, b.id))
      const part = items.map((message) => ({ id: message.info.id, part: sortMessageParts(message.parts) }))
      return {
        session,
        part,
        ...
        async sync(sessionID: string) {
          ...
          const sessionReq = hasSession
            ...
            : retry(() => client.session.get({ sessionID })).then((session) => {
                const data = (session as { data?: import("@rendesk/sdk/v2/client").Session }).data
                if (!data) return
                setStore(
                  "session",
                  produce((draft) => {
        ...
        async diff(sessionID: string) {
          ...
          return runInflight(inflightDiff, key, () =>
            retry(() => client.session.diff({ sessionID })).then((diff) => {
              const data = ((diff as { data?: any[] }).data ?? []) as any[]
        ...
        async todo(sessionID: string) {
          ...
          return runInflight(inflightTodo, key, () =>
            retry(() => client.session.todo({ sessionID })).then((todo) => {
              const list = (((todo as { data?: any[] }).data) ?? []) as any[]
