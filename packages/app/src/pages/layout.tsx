...
import { Session, type Message } from "@rendesk/sdk/v2/client"
...
import { showToast, Toast, toaster } from "@rendesk/ui/toast"
import { useGlobalSDK } from "@/context/global-sdk"
import { useNotification } from "@/context/notification"
import { usePermission } from "@/context/permission"
import { Binary } from "@rendesk/util/binary"
import { retry } from "@rendesk/util/retry"
  ...
  async function prefetchMessages(directory: string, sessionID: string, token: number) {
    ...
    return retry(() => globalSDK.client.session.messages({ directory, sessionID, limit: prefetchChunk }))
      .then((messages) => {
        ...
        const items = (((messages as { data?: Array<{ info: Message; parts: unknown[] }> }).data) ?? []).filter(
          (x) => !!x?.info?.id,
        )
        ...
        batch(() => {
          ...
          for (const message of items) {
            ...
            const mergedParts = sortMessageParts(
              mergeByID(
                currentParts.filter((item): item is (typeof currentParts)[number] & { id: string } => !!item?.id),
                (message.parts as Array<{ id: string }>).filter((item): item is { id: string } => !!item?.id),
              ) as any[],
            )
  ...
  command.register("layout", () => {
    const commands: CommandOption[] = [
      ...
      },
      {
        ...
        category: language.t("command.category.view"),
        keybind: "mod+shift+d",
        disabled: !dashboardEnabled() || !currentDir(),
        onSelect: () => {
          const directory = currentDir()
          if (!directory) return
        ...
        >
          <div class="@container w-full h-full contain-strict">
            <SidebarContent
              ...
              onOpenHelp={() => platform.openLink("https://github.com/renvel/rendesk/issues")}
              renderPanel={() => (
