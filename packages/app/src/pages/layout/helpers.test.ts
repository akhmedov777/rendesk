...
import { type Session } from "@rendesk/sdk/v2/client"
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("rendesk://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("rendesk://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("rendesk://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("rendesk://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("keeps legacy opencode deep links working", () => {
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("rendesk://open-project")).toBeUndefined()
    expect(parseDeepLink("rendesk://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "rendesk://open-project?directory=/a",
      "rendesk://other?directory=/b",
      "rendesk://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("rendesk://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("rendesk://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("rendesk://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("rendesk://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "rendesk://new-session?directory=/a",
      "rendesk://open-project?directory=/b",
      "rendesk://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __RENDESK__: {
        deepLinks: ["rendesk://open-project?directory=/a"],
      },
      __OPENCODE__: {
        deepLinks: ["opencode://open-project?directory=/b"],
      },
    } as unknown as Window & {
      __RENDESK__?: { deepLinks?: string[] }
      __OPENCODE__?: { deepLinks?: string[] }
    }

    expect(drainPendingDeepLinks(target)).toEqual(["rendesk://open-project?directory=/a"])
