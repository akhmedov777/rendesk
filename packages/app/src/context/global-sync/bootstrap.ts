  ...
  Promise.all([
    ...
    input.sdk.permission.list().then((x) => {
      const grouped = groupBySession(
        ((x.data ?? []) as PermissionRequest[]).filter((perm): perm is PermissionRequest => !!perm?.id && !!perm.sessionID),
      )
      batch(() => {
    ...
    input.sdk.question.list().then((x) => {
      const grouped = groupBySession(
        ((x.data ?? []) as QuestionRequest[]).filter((q): q is QuestionRequest => !!q?.id && !!q.sessionID),
      )
      batch(() => {
