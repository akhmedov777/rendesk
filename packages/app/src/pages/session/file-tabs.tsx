...
import type { FileSearchHandle } from "@rendesk/ui/file"
import { useFileComponent } from "@rendesk/ui/context/file"
import { cloneSelectedLineRange, previewSelectedLines } from "@rendesk/ui/pierre/selection-bridge"
import { createLineCommentController } from "@rendesk/ui/line-comment-annotations"
import { sampledChecksum } from "@rendesk/util/encode"
import { DropdownMenu } from "@rendesk/ui/dropdown-menu"
import { IconButton } from "@rendesk/ui/icon-button"
import { Tabs } from "@rendesk/ui/tabs"
import { ScrollView } from "@rendesk/ui/scroll-view"
...
const EMBEDDED_EDITOR_EXTENSIONS = new Set([
  "csv",
  "doc",
  "docx",
  "ods",
...
export function FileTabContent(props: { tab: string }) {
  ...
  const sessionDirectory = createMemo(() => decode64(params.dir) ?? "")
  const absoluteFilePath = createMemo(() => {
    const currentPath = path()
    const directory = sessionDirectory()
    if (!currentPath) return undefined
    if (currentPath.startsWith("/")) return currentPath
    if (/^[A-Za-z]:[\\/]/.test(currentPath) || /^[A-Za-z]:$/.test(currentPath)) return currentPath
    if (currentPath.startsWith("\\\\") || currentPath.startsWith("//")) return currentPath
    return `${directory.replace(/[\\/]+$/, "")}/${currentPath}`
  })
  const usesEmbeddedEditor = createMemo(() => {
    const ext = absoluteFilePath()?.split(".").pop()?.toLowerCase() ?? ""
    return EMBEDDED_EDITOR_EXTENSIONS.has(ext)
  })
  ...
  const renderFile = (source: string, options?: { fullHeight?: boolean }) => (
    <div
      classList={{
        "relative overflow-hidden pb-40": !options?.fullHeight,
        "relative h-full min-h-0 w-full min-w-0 overflow-hidden": !!options?.fullHeight,
      }}
    >
      <Dynamic
  ...
  return (
    <Tabs.Content value={props.tab} class="mt-3 relative flex h-full min-h-0 flex-col overflow-hidden contain-strict">
      <Switch>
        <Match when={state()?.loaded && usesEmbeddedEditor()}>
          {renderFile(contents(), { fullHeight: true })}
        </Match>
